...
void* update_medoids(void* args){
    ThreadArgs* thread = (ThreadArgs*)args;
    int newMedoidIdx;
    double minDistance, total;

    int* isChanged = new int;
    *isChanged = 0;

    for(int z = thread->start; z < thread -> end; z++){
        newMedoidIdx = -1;
        ...
        // medoid is changed for the cluster 
        if(medoidsIndex[z] != newMedoidIdx){
            medoidsIndex[z] = newMedoidIdx;
            // means medoid is changed
            *isChanged = 1;
        }
    }

    pthread_exit((void*)isChanged);

}

// assign medoids to every points parallely 
void* parallel_assign(void* args){
    ThreadArgs * thread = (ThreadArgs*)args;
...
int main(int argc, char* argv[]) {
    ...
    int a_chunkSize = N / threadNum;
    int a_remaining = N % threadNum;
    int start = 0;


    int b_chunkSize = K / threadNum;
    int b_remaining = K % threadNum;
    ...
    for(int i = 0; i < maxIter; i++){
        ...
        // 1. assign medoids to every points
        start = 0;
        for(int j = 0; j < threadNum; j++){
            args[j].start = start;
            args[j].end = start + a_chunkSize + (j < a_remaining ? 1 : 0 );
            pthread_create(&threads[j], NULL, parallel_assign, (void*)&args[j]);
            start = args[j].end;
        }
        ...
        for(int j = 0; j < threadNum; j++){
            args[j].start = start;
            args[j].end = start + b_chunkSize + ( j < b_remaining ? 1 : 0);
            pthread_create(&threads[j], NULL, update_medoids, (void*)&args[j]);
            start = args[j].end;
        }
